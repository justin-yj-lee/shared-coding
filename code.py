...
# concatenate using dictionary method
def solve(L):
	next={}
	for s in L:
		for i in range(0,len(s)-5):     # taking all elements in LL and creating a dictionary of all possible four letter sequences with the subsequent fifth letter becoming the entry value
			next[s[i:i+5]]=s[i+5]

	print (next)

	begin="MSKDT"      # the beginning four letters of sequence is assumed as given
	seq=begin
...
# test is final sequence is correct
def test(seq, L):
	for n in L:
		if n not in seq:
			return False
